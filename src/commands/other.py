...
async def info(m, c, arg):
    ...
    if page == ['']:
        ...
        if len(page) == 1:
            ...
            charData = c.jsonfiles['char'][page[1].lower().replace('&','').replace('é','e')]
...
async def ball8(m, c):
    ...
    while True:
        ...
        try:
            r, u = await c.wait_for('reaction_add', check=lambda r, u:  str(r.emoji) == "🔄" and r.message.id == ball8msg.id and u == m.author, timeout=240)
